...
- On those same harness aborts, write the failing JUnit XML first, then collect a compact `TEST_RESULTS_DIR/debug/failure-snapshot.txt` (deployments, pods, MLflow CR conditions, warning events, last server-info HTTP/body). Snapshot `kubectl` calls must use `--request-timeout` so a hung API cannot block the report. After a successful snapshot, replace only the harness JUnit file this run created (never a pre-existing pytest report) so left-nav includes the snapshot. Keep full pod logs in `collect-debug-logs.sh` artifacts. `deploy.py` failures must also run `collect-debug-logs.sh`. Server-info retries must log HTTP status, curl stderr, and a body preview on every attempt instead of only "retrying in 5s"
