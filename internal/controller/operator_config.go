...
func (r *MLflowReconciler) resolveOperatorConfigFromBase(
	...
	if module.Spec.Gateway != nil {
		if moduleURL := gatewayDomainToURL(module.Spec.Gateway.Domain); moduleURL != "" {
			base.MLflowURL = moduleURL
...
func (r *MLflowReconciler) ensureMLflowOperatorReady(
	...
	if ready {
		setMLflowOperatorDependencyCondition(mlflow, metav1.ConditionTrue, readyReason, "MLflowOperator is ready to manage MLflow custom resources", module)
		return ctrl.Result{}, false, nil
	}

	setMLflowOperatorDependencyCondition(mlflow, metav1.ConditionFalse, reason, message, module)
...
func setMLflowOperatorDependencyCondition(
	...
	if module != nil {
		condition.ObservedGeneration = module.Generation
	}
